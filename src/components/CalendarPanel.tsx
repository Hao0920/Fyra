  ...
  return (
    <div className="card overflow-hidden p-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <button className="rounded-full border border-slate-700/90 bg-slate-900/80 px-3 py-2 text-xs text-slate-300 transition hover:border-slate-500">
          周视图
        </button>
      </div>
      <div className="grid gap-2 rounded-[2rem] bg-slate-950/90 p-4 text-sm text-slate-300">
        ...
        <div className="grid grid-cols-7 gap-2 text-center text-sm">
          {Array.from({ length: 30 }, (_, index) => {
            ...
            return (
              <div
                key={day}
                className={`rounded-2xl border p-3 ${isActive ? 'border-sky-400 bg-sky-500/10 text-sky-200' : 'border-transparent bg-slate-900/80 text-slate-300'} ${isWeekend ? 'text-rose-300' : ''}`}
              >
      ...
      <div className="mt-6 rounded-[2rem] border border-slate-800/90 bg-slate-900/70 p-5 text-sm text-slate-300">
